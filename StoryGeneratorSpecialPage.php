...
class StoryGeneratorSpecialPage extends SpecialPage {
	private $fileName;
	public function __construct() {
		parent::__construct('StoryGenerator');

		$sess = MediaWiki\Session\SessionManager::getGlobalSession()->getId();
		if (strlen($sess)) {
			$this->fileName = $sess.".pdf";
		} else {
			$this->fileName = MediaWiki\Session\SessionManager::singleton()->generateSessionId().".pdf";
		}
	}

	function execute($query) {
		$this->setHeaders();
		$this->doStoryGenerator($query);
	}
	...
	function doStoryGenerator($query) {
		...
		$submitted = $request->getVal('submitted');
		if (!strlen($submitted)) $submitted = false;
	
		$output->setPageTitle("Story Generator");
		...
		if ($submitted) {
			...
			$planets = StoryGeneratorLib::getStoryCategory('Planets');
			...
			$ships = StoryGeneratorLib::getStoryCategory('Space_Vehicles');
			//print_r($ships);
			...
			$plots = StoryGeneratorLib::storyCopyResultsIntoArray($results);
			...
			$conflicts = StoryGeneratorLib::storyCopyResultsIntoArray($results);
			...
			$pdf->AddPage();
			$pdf->Image(dirname(dirname(dirname(dirname(__FILE__)))).'/wp-content/themes/new-urs-theme/img/urs-login.png', 70);
			...
			$pdf->WriteHTML($legal);
				$pdf->Ln(15);
			$pdf->Image(dirname(dirname(dirname(dirname(__FILE__)))).'/wp-content/themes/new-urs-theme/img/CC-BY-NC-SA.png', 93);
			$pdf->Output(dirname(__FILE__).'/cache/'.$this->fileName, 'F');
	
			$chmodval = chmod(dirname(__FILE__).'/cache/'.$this->fileName, 0777);
	
			$body .= '<p><b>Download this information in <a href="/wiki/extensions/StoryGenerator/cache/'.$this->fileName.'">a handy PDF</a> for future reference.</b></p>';
